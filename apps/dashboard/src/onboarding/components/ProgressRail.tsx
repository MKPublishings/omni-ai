...
export function ProgressRail({ currentStep, compact = false }: ProgressRailProps) {
  ...
  return (
    <GlassCard tier={2} className="rounded-[1.65rem] p-4 sm:rounded-[1.75rem] sm:p-6">
      ...
      <div className={clsx('mt-4 grid gap-3 sm:mt-5', compact ? 'grid-cols-1' : 'grid-cols-1')}>
        {ONBOARDING_STEPS.map((step, index) => {
          ...
          return (
            ...
            >
              ...
              {!compact ? <p className="mt-3 text-sm leading-6 text-quantum-white/64">{step.description}</p> : null}
            </article>
          )
        })}
      </div>
