export class IONSafety {
  ...
  static safeGuardResponse(text: string, maxChars: number = IONSafety.DEFAULT_MAX_RESPONSE_CHARS): string {
    const limit = Number.isFinite(maxChars) && maxChars > 0
      ? Math.floor(maxChars)
      : IONSafety.DEFAULT_MAX_RESPONSE_CHARS;
