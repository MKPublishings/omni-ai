...
function scoreRouteCandidates(text: string, mode: string): { candidates: RouteScore[]; signals: string[] } {
  ...
  if (
    hasAny(lower, [
      /\b(generate image|create image|render image|make an image|visualize this|image prompt|art prompt|illustration of|photo of|thumbnail|storyboard frame|concept frame)\b/i
    ])
  ) {
