...
export function normalizeImageGenerationError(err: any): { status: number; code: string; message: string; details?: string } {
  ...
  if (rawCode === "E_SAFETY_BLOCK") return { status: 403, code: "safety-blocked", message: "Prompt violated content policy.", details: rawMessage || undefined };
  ...
  if (value.includes("moderat") || value.includes("safety") || value.includes("policy") || value.includes("nsfw") || value.includes("unsafe") || value.includes("content blocked")) return { status: 422, code: "provider-policy-blocked", message: "Image provider rejected this prompt under policy constraints.", details: rawMessage || undefined };
